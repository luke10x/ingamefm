...
xfm_wav_voice_id xfm_wav_sfx_play(xfm_wav_module* m, int sfx_id, int priority)
{
    ...
    // Stop any existing SFX on this voice before starting new one
    // This matches the synth behavior where key_off() is called before key_on()
    if (m->active_sfx[voice].active) {
        // Voice was playing something - stop it first
